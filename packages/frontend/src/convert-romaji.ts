// それなりにありそうなローマ字表記を列挙する
export function convertToRomaji(hiragana: string):string[]{
  // bmp前のローマ字をmにするやつはあんまおらんやろ。長音も基本的にそんなに入力されることないから、まあ適当で。
  const hepburnRawResult = kanaToRoman(hiragana, 'hepburn', {bmp: false});
  const kunreiRawResult = kanaToRoman(hiragana, 'kunrei', {bmp: false});
